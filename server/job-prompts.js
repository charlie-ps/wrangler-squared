...
export function jobPrompt(job, sub, run) {
  ...
  const prompts = {
    planning: lines(
      ...
      '`after` on a PR means DEPLOY AFTER; `after` on a session means START AFTER (its output is an input). Minimise dependencies and PRs while keeping each piece independently deliverable. Do not propose branch names: the implementing session names its own.',
