...
export function initJobsView({ send, getAgents, getTasks = () => [], onSession, onDiff, onBoard }) {
  ...
  // Snapshots written before the setting existed carry no `ideApp`.
  const ideApp = () => data.settings.ideApp || 'IntelliJ IDEA';
  ...
  function renderDetail() {
    ...
    if (planDraft) {
      ...
      body = `<p class="job-detail-meta">${kindChipHtml(sub)} ${esc(sub.jiraKey || (sub.storyId ? 'Ticket pending' : 'No ticket'))} · Agent session on this machine</p>${deps.length ? `<div class="job-dependency-list">${deps.map((d) => `<span>${dependencySatisfied(d) ? '✓' : d?.cancelledAt ? '×' : '↳'} After ${esc(d?.title)}${d?.cancelledAt ? ' (dropped)' : ''}</span>`).join('')}</div>` : ''}
    ...
    } else body = `<p class="job-intent">${esc(job.intent)}</p><p>${job.repos.length ? job.repos.map((r) => esc(tildify(r))).join('<br>') : 'Wrangler will discover the repositories needed during planning.'}</p>${cancelledHtml(job)}${job.cancelledAt && !job.subJobs.length ? '' : job.stage === 'backlog' ? `<p class="job-authority">Planning proposes Jira story titles without touching Jira. You review the plan before tickets are created or work begins.</p><button class="primary" data-action="start">Start planning</button>` : job.stage === 'jira' ? `${contextHtml(job.plan)}<h3>Approved stories</h3><div class="job-stories">${job.plan.stories.map((s) => `<div><b>${esc(storyLabel(s, job.plan.stories))} · ${esc(s.title)}</b></div>`).join('')}</div><p class="job-authority">Creating the approved Jira stories. Work starts once every story has a key.</p><h3>Approved work</h3>${planGraphHtml(job.plan)}` : job.stage === 'active' || job.stage === 'done' ? `${contextHtml(job.plan)}${job.plan?.stories?.length ? `<h3>Stories <small>${job.plan.stories.length}</small></h3><div class="job-stories">${job.plan.stories.map((s) => `<div><b>${esc(storyLabel(s, job.plan.stories))} · ${esc(s.title)}</b></div>`).join('')}</div>` : ''}<h3>Sub-jobs <small>${esc(kindCountLabel(job.subJobs))}</small></h3>${planGraphHtml({ subJobs: job.subJobs, stories: job.plan?.stories }, { statusOf: (s) => jobStatus(job, s) })}${movesHistoryHtml(job)}` : job.cancelledAt ? '' : '<p>Wrangler will bring the plan here for review.</p>'}`;
    ...
    // The IDE opens beside the board rather than instead of it, so the dialog
    // stays: approving is still done here once the code has been read.
    bind('#job-ide', () => send({ type: 'job-open-ide', id: job.id, subJobId: sub.id }));
  }
  ...
  function createJob() {
    ...
    form.elements.taskId.onchange = () => {
      const creating = form.elements.taskId.value === '__new__';
      form.elements.newTaskName.hidden = !creating;
      form.elements.newTaskName.required = creating;
      form.elements.newTaskName.closest('label').hidden = !creating;
    };
    form.onsubmit = (e) => {
      ...
      send({ type: 'job-create', start: e.submitter?.value !== 'backlog', ...only({ newTaskName: taskId === '__new__' ? String(values.get('newTaskName')).trim() : '' }), job: { title: values.get('title'), intent: values.get('intent'), repos: [...new Set(values.get('repos').split('\n').map((s) => s.trim()).filter(Boolean))], agent: values.get('agent'), model: values.get('model') || '', planningPrompt: values.get('planningPrompt'), reviewCode: values.has('reviewCode'), reviewMerge: values.has('reviewMerge'), reviewSessions: values.has('reviewSessions'), updateMain: values.has('updateMain'), taskId: taskId && taskId !== '__new__' ? taskId : null } });
    };
  }
